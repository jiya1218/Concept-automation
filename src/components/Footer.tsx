...
export function Footer() {
  ...
  return (
    <>
      <footer className="mt-20 bg-[#0B132B] text-slate-300 border-t border-slate-800">
        {/* Top Accent Bar */}
        <div className="h-1 w-full bg-gradient-to-r from-sky-500 via-blue-600 to-sky-400" />
            ...
            {subscribed ? (
              ...
              <form onSubmit={handleNewsletter} className="flex w-full max-w-md items-center gap-2">
                <input
                  ...
                  className="w-full rounded-xl border border-slate-700 bg-slate-900 px-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:border-sky-500 focus:outline-none"
                />
                <button
                  type="submit"
                  className="inline-flex items-center gap-1.5 shrink-0 rounded-xl bg-sky-600 px-5 py-2.5 font-display text-xs font-bold uppercase tracking-wider text-white hover:bg-sky-500 transition-colors shadow-md"
                >
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:px-8 py-12 md:grid-cols-2 lg:grid-cols-5">
          {/* Col 1: Brand Info */}
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-center gap-3">
              <img
                ...
                className="h-12 w-12 rounded-xl object-contain bg-white p-1 border border-slate-800 shadow-md"
                width={48}
                height={48}
              />
              <span className="font-display text-base sm:text-lg font-extrabold uppercase leading-tight text-white">
                Concept Automation
                <br />
                Technologies
              </span>
            ...
            <p className="text-xs uppercase tracking-widest text-sky-400 font-bold flex items-center gap-1.5 font-mono">
            ...
            <div className="flex flex-wrap gap-2.5 pt-1">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-sky-500/10 px-4 py-2 text-xs font-bold uppercase tracking-wider text-sky-400 border border-sky-500/30 hover:bg-sky-600 hover:text-white transition-all"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1.5 rounded-xl bg-emerald-500/10 px-4 py-2 text-xs font-bold uppercase tracking-wider text-emerald-400 border border-emerald-500/30 hover:bg-emerald-600 hover:text-white transition-all"
              >
                WhatsApp Us
              </a>
            </div>
              ...
              {POPULAR_CATEGORIES.map((cat) => (
                <li key={cat.name}>
                  <Link
                    to="/products"
                    search={{ q: cat.query }}
                    className="hover:text-sky-400 transition-colors block text-[11px]"
                  >
                    {cat.name}
                  </Link>
                </li>
          ...
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-1.5 text-xs">
              {brands.map((brand) => (
                <Link
                  ...
                  to="/products"
                  search={{ q: brand }}
                  className="rounded-lg bg-slate-900 px-2 py-1 text-[11px] text-slate-300 border border-slate-800 hover:bg-sky-600 hover:text-white transition-colors"
                >
                  {brand}
                </Link>
              ))}
            ...
            <h3 className="mt-6 font-display text-xs font-bold uppercase tracking-[0.2em] text-sky-400">
              Headquarters
            </h3>
            <ul className="mt-3 space-y-2 text-xs text-slate-400">
              <li className="flex items-start gap-2">
                <MapPin className="h-3.5 w-3.5 shrink-0 text-sky-400 mt-0.5" />
                <a
                  href="https://maps.google.com/?q=Titanium+Business+Park+Makarba+Ahmedabad"
                  target="_blank"
                  rel="noreferrer"
                  className="hover:text-sky-400 leading-snug transition-colors flex items-center gap-1"
                >
              ...
              <li className="flex items-center gap-2">
                ...
                <a href={`tel:${company.phoneRaw}`} className="hover:text-sky-400 font-mono text-[11px]">
                  {company.phone}
                </a>
              </li>
              <li className="flex items-center gap-2">
                <Mail className="h-3.5 w-3.5 shrink-0 text-sky-400" />
                <a href={`mailto:${company.email}`} className="hover:text-sky-400 font-mono text-[11px]">
                  {company.email}
                </a>
              </li>
