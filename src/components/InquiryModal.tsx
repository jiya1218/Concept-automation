...
export function InquiryModal({ isOpen, onClose, productName = "", partNumber = "" }: InquiryModalProps) {
  ...
  const inputClass = "w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-xs text-[#0f2a4a] placeholder-slate-400 focus:border-[#0f2a4a] focus:outline-none focus:ring-1 focus:ring-[#0f2a4a]";

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0a1e36]/60 p-4 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl animate-scale-in" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="bg-[#0a1e36] px-6 py-4 text-white flex items-center justify-between">
        ...
        <div className="p-5 sm:p-6 max-h-[75vh] overflow-y-auto">
          {submitted ? (
            <div className="py-6 text-center">
              ...
              <h3 className="mt-3 text-base font-bold font-display uppercase tracking-wider text-[#0f2a4a]">Quotation Request Received!</h3>
              <p className="mt-2 text-xs text-slate-500">
                Thank you, <strong className="text-[#0f2a4a]">{formData.name}</strong>. We'll email/call you shortly.
              </p>
              <button
                onClick={() => { setSubmitted(false); onClose(); }}
                className="mt-5 w-full rounded-lg bg-[#0f2a4a] py-2.5 text-xs font-bold uppercase tracking-wider text-white hover:bg-[#0a1e36] transition-colors"
              >
                Done
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex h-10 w-28 shrink-0 items-center justify-center rounded-lg bg-[#0f2a4a] font-mono text-sm font-bold tracking-widest text-white select-none">
                  {captchaCode}
                </div>
              </div>

              <button type="submit" disabled={loading}
                className="w-full flex items-center justify-center gap-2 rounded-lg bg-[#0f2a4a] py-3 text-xs font-bold uppercase tracking-wider text-white hover:bg-[#0a1e36] transition-colors disabled:opacity-50"
              >
                {loading ? "Submitting..." : <><Send className="h-4 w-4 text-[#d97706]" /> Submit Inquiry</>}
              </button>
